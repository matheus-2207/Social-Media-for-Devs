...
export function UserSearch({ viewerId }: { viewerId: string }) {
  ...
  return <section className="mt-6" aria-label="Buscar pessoas">
    ...
    <input id="user-search" type="search" value={query} maxLength={100} onChange={event => setQuery(event.target.value)} placeholder="@username ou nome" autoComplete="off" className="input-control mt-2 w-full" />
    <div aria-live="polite" className="mt-5 text-sm text-subtle">
      ...
      {phase === "error" && <p role="alert">Não foi possível buscar usuários. <button type="button" onClick={() => setRetry(value => value + 1)} className="text-accent underline">Tentar novamente</button></p>}
    </div>
    {phase === "done" && users.length > 0 && <><ul className="mt-5 space-y-3">{users.map(user => <li key={user.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-line bg-surface p-5 text-ink hover:border-control">
    ...
    </li>)}</ul>{users.length === 20 && <p className="mt-4 text-sm text-subtle">Mostrando até 20 pessoas. Refine a busca para encontrar outras.</p>}</>}
  </section>;
}
